...
type CollaborativeEditorHandle = {
  getMdx: () => string
  insertMdx: (mdx: string, inline?: boolean) => void
  focus: () => void
}
...
function syncActiveEditorContent(): string {
  const latestContent = editorRef.value?.getMdx() ?? editContent.value
...
function onEditorDrop(e: DragEvent) {
  ...
  editorRef.value?.insertMdx(syntax, true)
  draggingId.value = null
  dropTargetId.value = null
}
...
function inferNodeTypeFromName(name: string): { type: ApiNode['type']; kind: string } {
  ...
  // 文档
  if (/\.(mdx?|txt|doc|docx|pdf|json|log)$/.test(lower)) return { type: 'doc', kind: 'text' }
  // 图片
...
const menuItems = computed(() => {
  ...
  if (contextMenu.value.type === 'collection') {
    return [
      { label: 'New Doc', svg: svg('doc'), action: () => { if (targetId) createNode('temp.mdx', targetId) } },
    ...
    <aside class="sidebar panel">
      ...
      <section class="sidebar-block">
        ...
        <div class="tree-toolbar">
          <button type="button" class="toolbar-btn" title="New File" @click="createNode('temp.mdx')">
